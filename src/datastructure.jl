...
start node index `iₛ`, end node index `iₑ`, start time `tₛ`, end time `tₑ`, number 
of customers `n`, load `q`, and length `l`.
"""
mutable struct Route
    ...
    tₛ::Float64                                                                     # Route start time (departure time from the depot node)
    tₑ::Float64                                                                     # Route end time (subsequent arrival time at the depot node)
...
index `o`, capacity `q`, speed `s`, service time `τᵈ` at depot node 
and `τᶜ` at customer node, operational cost `πₒ`, and set of routes `R`.
"""
struct Vehicle
...
A `CustomerNode` is a sink point on the graph at `(x,y)` with index `i`, demand `q`, earliest 
service time `tₑ`, latest service time `tₗ`, tail node index `iₜ`, head node index `iₕ`, 
arrival time `tₐ`, departure time `tᵈ`, on route `r`.
"""
mutable struct CustomerNode <: Node
    ...
    tₑ::Float64                                                                     # Customer node earliest service time
    tₗ::Float64                                                                     # Customer node latest service time
    iₜ::Int64                                                                       # Tail (predecessor) node index
    iₕ::Int64                                                                       # Head (successor) node index
    tₐ::Float64                                                                     # Customer node arrival time
...
# Null route
const NullRoute = Route(0, 0, 0, 0, Inf, Inf, 0, 0, Inf) 

# Empty (closed) route traversed by vehicle v from depot d
Route(i, v::Vehicle, d::DepotNode) = Route(i, v.i, d.i, d.i, 0., 0., 0, 0, 0)               
